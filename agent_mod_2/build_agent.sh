...
if [ "$1" = "--clean" ]; then
    ...
    docker build --no-cache --platform linux/amd64 -f Dockerfile-base -t cvdp-agent-mod-2-base .
else
    echo "Fast build: using cached base..."
    docker build --platform linux/amd64 -f Dockerfile-base -t cvdp-agent-mod-2-base .
fi

docker build --no-cache --platform linux/amd64 -f Dockerfile-agent -t cvdp-agent-mod-2 .